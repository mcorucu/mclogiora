...
Nothing here is loaded or reachable unless a site owner enables the feature, selects
a provider and supplies a credential.
...
**OpenAI** — Responses API at `/v1/responses`. Every request sends `store => false`
from the `REQUIRED_STORE` policy constant; a unit test asserts the key exists and is
boolean `false`, not merely falsy. No `previous_response_id`, `conversation`,
`background`, tools, files or attachments. Model ids are filtered to the families the
adapter can use.

**Anthropic** — Messages API at `/v1/messages` with an explicit `anthropic-version`
header. Provider-specific request and response translation is confined to the
adapter.

**Gemini** — `generateContent` on the v1beta endpoint. The key travels in the
`x-goog-api-key` header rather than a `?key=` query parameter, so it does not appear
in server access logs as part of a URL. The response envelope is unwrapped inside
the adapter.
...
| Suite | Scope |
| --- | --- |
| Unit | Provider adapters against recorded response shapes, placeholder shielding, service readiness, status transitions. Includes the OpenAI `store => false` invariant. |
...
OpenAI, Anthropic, Google or DeepL with a real credential.
