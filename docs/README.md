...
Current phase: Phase 18, Slice 2, Performance, Accessibility, RTL & WordPress.org Compliance Hardening, complete. Phases 02 through 17 are complete; the current pre-submission version line is v1.0.2. WordPress.org submission remains a separate manual step.

Language management and relation records persist. Translation workflows cover posts, pages, public custom post types, public taxonomies, interface strings, media metadata, navigation menus, and supported widgets. Phase 12 decides which of them a given front-end request renders, Phase 13 describes that decision to search engines, Phases 14 and 15 provide the editor and builder surfaces, and Phase 16 adds review-only translation suggestions over WordPress AI Client or a dedicated translation service. Phase 17 adds the developer and operations layer: the public read API, hook contracts, REST endpoints, WP-CLI, import/export, and read-only System Status/Site Health diagnostics. The bundled language catalog and local manual are documented in `architecture/language-catalog.md` and remain offline-capable.
